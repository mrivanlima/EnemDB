...
BEGIN
    ...
    IF p_degree_level_name IS NULL OR length(trim(p_degree_level_name)) = 0 THEN
        out_message := 'Validation failed: degree_level_name cannot be empty.';
        RETURN;
    END IF;
    ...
    -- Uniqueness: degree_level_name
    SELECT 1 INTO v_exists FROM app.degree_level WHERE degree_level_name = p_degree_level_name;
    IF FOUND THEN
        out_message := format('Validation failed: degree_level_name "%s" already exists.', p_degree_level_name);
        RETURN;
    END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            v_command := format(
                'CALL app.usp_api_degree_level_create(p_degree_level_name => %L, p_created_by => %L)',
